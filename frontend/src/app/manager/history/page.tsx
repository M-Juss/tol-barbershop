...
type Status = Extract<AppointmentStatus, "Completed" | "Rejected">;
...
const tabs: Status[] = ["Completed", "Rejected"];

function toUiStatus(status: Appointment["status"]): Status | null {
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const mapped = data
          .map((appt) => {
            const mappedStatus = toUiStatus(appt.status);
          ...
          .filter((appt): appt is UiAppointment => appt !== null);
    ...
    <div className="w-full h-full bg-slate-100 p-4 sm:p-6 font-sans">
      ...
      <div className="bg-white rounded-xl flex flex-wrap p-1 gap-1 mb-4 shadow-sm border border-gray-100 w-full sm:w-fit">
        {tabs.map((tab) => (
          ...
          >
            {tab === "Rejected" ? "Cancelled" : tab} ({countByStatus(tab)})
          </button>
        ))}
      </div>
      ...
      <div className="flex flex-col gap-3">
        {loading ? (
          ...
          <div className="bg-white rounded-xl p-10 text-center text-gray-400 border border-gray-100 shadow-sm">
            No {activeTab === "Rejected" ? "cancelled" : "completed"} appointments.
          </div>
        ) : (
          filtered.map((apt) => (
            <AppointmentCard
